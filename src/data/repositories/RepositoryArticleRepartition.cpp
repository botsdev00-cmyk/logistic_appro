...
bool RepositoryArticleRepartition::create(const ArticleRepartition& article) {
    ...
    if (!query.exec()) {
        m_dernierErreur = "Erreur création article_repartition : " + query.lastError().text();
        return false;
    }
    return true;
}
...
bool RepositoryArticleRepartition::update(const ArticleRepartition& article) {
    ...
    query.prepare(
        ...
        "sync_status=?, version=?, updated_at=?, updated_by=? "
    ...
    query.addBindValue(article.getArticleRepartitionId().toString(QUuid::WithoutBraces));
    if (!query.exec()) {
        m_dernierErreur = "Erreur mise à jour article_repartition : " + query.lastError().text();
        return false;
    }
...
bool RepositoryArticleRepartition::remove(const QUuid& articleId) {
    ...
    if (!query.exec()) {
        m_dernierErreur = "Erreur suppression article_repartition : " + query.lastError().text();
        return false;
    }
    return query.numRowsAffected() > 0;
}
...
ArticleRepartition RepositoryArticleRepartition::getById(const QUuid& id) const {
    ConnexionBaseDonnees& bd = ConnexionBaseDonnees::getInstance();
    QSqlQuery query(bd.getDatabase());
    query.prepare("SELECT * FROM articles_repartition WHERE article_repartition_id=? AND deleted_at IS NULL");
...
QList<ArticleRepartition> RepositoryArticleRepartition::getAll() const {
    ...
    if (query.exec("SELECT * FROM articles_repartition WHERE deleted_at IS NULL ORDER BY created_at DESC"))
        while (query.next()) articles.append(mapRowToArticle(query));
    return articles;
}
...
QList<ArticleRepartition> RepositoryArticleRepartition::getByRepartitionId(const QUuid& repartitionId) const {
    ...
    QSqlQuery query(bd.getDatabase());
    QList<ArticleRepartition> articles;
    query.prepare("SELECT * FROM articles_repartition WHERE repartition_id=? AND deleted_at IS NULL ORDER BY created_at DESC");
    query.addBindValue(repartitionId.toString(QUuid::WithoutBraces));
    if (query.exec()) while (query.next()) articles.append(mapRowToArticle(query));
    return articles;
}
...
QList<ArticleRepartition> RepositoryArticleRepartition::getPendingSync() const {
    ...
    QSqlQuery query(bd.getDatabase());
    QList<ArticleRepartition> articles;
    if (query.exec("SELECT * FROM articles_repartition WHERE sync_status='PENDING' AND deleted_at IS NULL ORDER BY updated_at ASC"))
        while (query.next()) articles.append(mapRowToArticle(query));
    return articles;
}
...
QList<ArticleRepartition> RepositoryArticleRepartition::getSyncedArticles() const {
    ...
    QSqlQuery query(bd.getDatabase());
    QList<ArticleRepartition> articles;
    if (query.exec("SELECT * FROM articles_repartition WHERE sync_status='SYNCED' AND deleted_at IS NULL ORDER BY updated_at DESC"))
        while (query.next()) articles.append(mapRowToArticle(query));
    return articles;
}
...
QList<ArticleRepartition> RepositoryArticleRepartition::getConflictSync() const {
    ...
    QSqlQuery query(bd.getDatabase());
    QList<ArticleRepartition> articles;
    if (query.exec("SELECT * FROM articles_repartition WHERE sync_status='CONFLICT' AND deleted_at IS NULL ORDER BY updated_at ASC"))
        while (query.next()) articles.append(mapRowToArticle(query));
    return articles;
}
...
QList<ArticleRepartition> RepositoryArticleRepartition::getSinceVersion(int minVersion) const {
    ...
    QSqlQuery query(bd.getDatabase());
    QList<ArticleRepartition> articles;
    query.prepare("SELECT * FROM articles_repartition WHERE version >= ? AND deleted_at IS NULL ORDER BY version ASC");
...
int RepositoryArticleRepartition::getPendingCount() const {
    ConnexionBaseDonnees& bd = ConnexionBaseDonnees::getInstance();
    QSqlQuery query(bd.getDatabase());
    if (query.exec("SELECT COUNT(*) FROM articles_repartition WHERE sync_status IN ('PENDING','CONFLICT') AND deleted_at IS NULL"))
        if (query.next()) return query.value(0).toInt();
    return 0;
}

QList<RepositoryArticleRepartition::SyncResult> RepositoryArticleRepartition::syncBatch(const QList<ArticleRepartition>& articles, const QUuid& utilisateurId) {
    ConnexionBaseDonnees& bd = ConnexionBaseDonnees::getInstance();
    QList<SyncResult> results;
    for (const ArticleRepartition& article : articles) {
        QSqlQuery query(bd.getDatabase());
        query.prepare("SELECT version FROM articles_repartition WHERE article_repartition_id = ? AND deleted_at IS NULL");
        query.addBindValue(article.getArticleRepartitionId().toString(QUuid::WithoutBraces));
        SyncResult res;
        ...
        upsertQuery.prepare(
            "INSERT INTO articles_repartition "
            "(article_repartition_id, repartition_id, produit_id, quantite_vente, quantite_cadeau, "
            "quantite_degustation, observation, sync_status, version, created_at, updated_at, created_by, updated_by) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, 'SYNCED', ?, ?, NOW(), ?, ?) "
            ...
            "sync_status='SYNCED', version=?, updated_at=NOW(), updated_by=?"
            );
